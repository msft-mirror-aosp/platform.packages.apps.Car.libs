...
internal class ContentProviderBrokerFactory(
  ...
  private fun refreshCompatibleApplicationInternal(
    ...
    compatibleProviderResolveInfos.forEach(Consumer { resolveInfo: ResolveInfo ->
      ...
      try {
        ...
        contentProviderClient?.let {
          result[identifier] = ContentProviderBroker(
            identifier,
            contentResolver,
        ...
        Log.e(
          TAG,
          "Exception occurred when trying to connect to content provider: $e"
        )
      }
    })
    return result
