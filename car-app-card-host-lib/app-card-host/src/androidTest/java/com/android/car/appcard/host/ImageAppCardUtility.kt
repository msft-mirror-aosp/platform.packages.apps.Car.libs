...
class ImageAppCardUtility {
  companion object {
    ...
    val button = Button.newBuilder(
      ...
      .setText(TEST_TITLE)
      .build()
    val header = Header.newBuilder(TEST_HEADER_COMPONENT_ID)
      .setTitle(TEST_TITLE)
      .build()
    val progressBar = ProgressBar.newBuilder(TEST_PROGRESS_BAR_COMPONENT_ID, min = 0, max = 2)
      .setProgress(1)
      .build()
    val image = Image.newBuilder(TEST_IMAGE_COMPONENT_ID)
      .setImageData(getSampleBitmap())
      .build()
    val imageAppCard = ImageAppCard.newBuilder(TEST_ID)
      .setImage(image)
      .setHeader(header)
      .setPrimaryText(TEST_PRIMARY_TEXT)
      .setSecondaryText(TEST_SECONDARY_TEXT)
      .build()
    val progressBarButtonCard = ImageAppCard.newBuilder(TEST_ID)
      .setHeader(header)
      .setPrimaryText(TEST_PRIMARY_TEXT)
      .setSecondaryText(TEST_SECONDARY_TEXT)
