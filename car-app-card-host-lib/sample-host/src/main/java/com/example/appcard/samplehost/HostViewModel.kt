...
class HostViewModel(application: Application) : AndroidViewModel(application), AppCardListener {
    ...
    /**
     * Keep track of the latest [AppCardContext] (if all [AppCard]s will have the same context) or
     * associate each [AppCard] with its own [AppCardContext]
     */
    ...
    /**
     * A new [AppCardContainer] has been received, either update an existing view or create a new
     * view representing the [AppCardContainer]
     */
    override fun onAppCardReceived(appCard: AppCardContainer) {
        ...
        synchronized(lock = allAppCards) {
            ...
            if (!updated) {
                stateFactory.getState(appCard)?.let { allAppCards.put(getKey(appCard), it) }
            }
        }
    }
    ...
    override fun onComponentReceived(component: AppCardComponentContainer) {
        ...
        synchronized(lock = allAppCards) { allAppCards[getKey(component)]?.update(component) }
    }
    ...
    /** An error has occurred when communicating with an [AppCardContentProvider] Handle error */
    ...
    private fun removeAllAppCards() {
        synchronized(lock = allAppCards) {
            allAppCards.forEach {
                appCardHost.notifyAppCardRemoved(it.value.identifier.value, it.value.appCardId)
            }
