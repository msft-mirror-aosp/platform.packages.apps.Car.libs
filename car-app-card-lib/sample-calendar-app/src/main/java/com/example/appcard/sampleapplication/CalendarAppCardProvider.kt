...
class CalendarAppCardProvider(
  ...
  fun getAppCard(appCardContext: AppCardContext): ImageAppCard {
    ...
    calculatedIsInteractable = isInteractable && latestAppCardContext.isInteractable &&
    ...
    if (!timerSetup.getAndSet(true)) {
      ...
      }, msLeftInCurrMinute, MINUTE_IN_MS)
    }
  ...
  private fun getImage(ctx: AppCardContext): Image {
    ...
    return Image.newBuilder(IMAGE_COMPONENT_ID)
      .setImageData(getTextAsImage(size))
      .build()
  }
  ...
  private fun getTextAsImage(size: Size): Bitmap {
    ...
    val textSizeModifier = if (clockMode) {
  ...
  private fun getPrimaryButton(ctx: AppCardContext): Button {
    ...
    val icon = if (clockMode) {
      resToBitmap(R.drawable.ic_calendar, size.width, size.height)
    } else {
      resToBitmap(R.drawable.ic_clock, size.width, size.height)
    }

    val builder = Button.newBuilder(
      PRIMARY_BUTTON_COMPONENT_ID,
      if (imageButtonNoBackground) Button.ButtonType.NO_BACKGROUND else Button.ButtonType.PRIMARY,
    ...
    if (!imageButtonNoBackground) builder.setText(if (clockMode) CALENDAR_TITLE else CLOCK_TITLE)
  ...
  private fun getSecondaryButton(ctx: AppCardContext): Button {
    return Button.newBuilder(
      ...
      if (imageButtonNoBackground) Button.ButtonType.PRIMARY else Button.ButtonType.SECONDARY,
      object : OnClickListener {
        override fun onClick() {
          // Switch mode when button is clicked
          is24hr = !is24hr
