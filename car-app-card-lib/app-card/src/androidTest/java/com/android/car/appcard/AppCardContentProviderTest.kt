...
class AppCardContentProviderTest {
    ...
    private val context = mock<Context> { on { contentResolver } doReturn contentResolver }
    ...
    fun setup() {
        ...
        image =
            Image.newBuilder(TEST_IMAGE_COMPONENT_ID)
                .setImageData(getSampleBitmap(strokeWidth = 3))
                .build()

        header = Header.newBuilder(TEST_HEADER_COMPONENT_ID).setTitle(TEST_TITLE).build()

        progressBar = newBuilder(TEST_PROGRESS_BAR_COMPONENT_ID, min = 0, max = 2).build()
        ...
        button =
            newBuilder(TEST_BUTTON_COMPONENT_ID, Button.ButtonType.PRIMARY, onClickListener)
                .setText(TEST_TITLE)
                .build()

        imageAppCardWithImageAndText =
            newBuilder(TEST_ID_WITH_IMAGE)
                .setImage(image)
                .setHeader(header)
        ...
        imageAppCardWithProgressBarButtons =
            newBuilder(TEST_ID_WITH_PROGRESS_BUTTONS)
                .setHeader(header)
                .setPrimaryText(TEST_PRIMARY_TEXT)
                .setSecondaryText(TEST_SECONDARY_TEXT)
    ...
    fun testUpdate_returnZero() {
        ...
        val result =
            testAppCardContentProvider.update(Uri.Builder().build(), contentValues, s, strings)
    ...
    fun testQuery_sendAllAppCards_noError_correctAppCards() {
        ...
        assertThat(secondAppCard?.toMessage())
            .isEqualTo(imageAppCardWithProgressBarButtons.toMessage())
    }
    ...
    fun testCall_appCardUpdate_noError_correctAppCard() {
        ...
        imageAppCardWithImageAndText =
            newBuilder(imageAppCardWithImageAndText)
    ...
    fun testCall_appCardComponentUpdate_noError_correctAppCard() {
        ...
        val progressBarMessage =
            ProgressBarMessage.newBuilder(progressBar.toMessage()).setProgress(1).build()
        ...
        val bundle = testAppCardContentProvider.call(MSG_APP_CARD_COMPONENT_UPDATE, NO_ARGS, b)
    ...
    fun testCall_appCardComponentUpdate_missingId_returnNull() {
        val bundle = Bundle()
        bundle.putString(
            AppCardContentProvider.BUNDLE_KEY_APP_CARD_ID,
            TEST_ID_WITH_PROGRESS_BUTTONS,
        )
        bundle.putBundle(AppCardContentProvider.BUNDLE_KEY_APP_CARD_CONTEXT, appCardContextBundle)
        val uri = Uri.Builder().authority(TEST_AUTHORITY).appendPath(MSG_APP_CARD_ADDED).build()
        ...
        val result = testAppCardContentProvider.call(MSG_APP_CARD_COMPONENT_UPDATE, NO_ARGS, bundle)
    ...
    fun testSendAppCardComponentUpdate_noExistingComponentId_notifyChangeNotCalled() {
        ...
        testAppCardContentProvider.query(uri, NO_ARGS, bundle, NO_ARGS)
        val progressBarMessage =
            ProgressBarMessage.newBuilder(progressBar.toMessage())
    ...
    fun testSendAppCardComponentUpdate_notEnforceFastUpdateRate_buttonUpdated() {
        ...
        val buttonMessage =
            com.android.car.appcard.internal.proto.Button.ButtonMessage.newBuilder(
                    button.toMessage()
                )
    ...
    fun testSendAppCardComponentUpdate_notEnforceFastUpdateRate_headerUpdated() {
        ...
        val message =
            HeaderMessage.newBuilder(header.toMessage()).setTitle(TEST_SECONDARY_TEXT).build()
    ...
    fun testSendAppCardComponentUpdate_notEnforceFastUpdateRate_imageUpdated() {
        ...
        var uri = Uri.Builder().authority(TEST_AUTHORITY).appendPath(MSG_APP_CARD_ADDED).build()
        testAppCardContentProvider.query(uri, NO_ARGS, b, NO_ARGS)
        assertThat(imageAppCardWithImageAndText.image?.contentScale)
            .isEqualTo(Image.ContentScale.FIT)
        val message =
            ImageMessage.newBuilder(image.toMessage())
                    ...
                    val blob =
                        it.getBlob(
                            it.getColumnIndexOrThrow(
                                AppCardContentProvider.CURSOR_COLUMN_APP_CARD_TRANSPORT
                            )
                        )
    ...
    private class TestAppCardContentProvider(
        ...
        override fun onAppCardAdded(id: String, ctx: AppCardContext): AppCard {
            ...
            appCards.forEach { if (it.id == this.id) appCard = it }
            return appCard!!
        }
    ...
    companion object {
        ...
        private val TEST_IMAGE_APP_CARD_CONTEXT =
            AppCardContext.ImageAppCardContext(
