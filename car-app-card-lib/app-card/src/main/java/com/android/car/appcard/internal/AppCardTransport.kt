...
class AppCardTransport : Parcelable {
  ...
  val appCard: AppCard?
     ...
     */
    get() = if (ImageAppCard::class.simpleName == className) {
  ...
  companion object {
    ...
    val CREATOR: Parcelable.Creator<AppCardTransport> =
      object : Parcelable.Creator<AppCardTransport> {
        override fun createFromParcel(source: Parcel): AppCardTransport = AppCardTransport(source)

        override fun newArray(size: Int): Array<AppCardTransport?> = arrayOfNulls(size)
      }

    @VisibleForTesting
    const val SUPPORTED_SERIALIZATION_VERSION = 1
  }
}
