...
/**
 * Provides application developers with certain contextual information on how their app
 * cards are going to be displayed.
 */
class AppCardContext(
  ...
  class ImageAppCardContext(
    ...
    /**
     * Get maximum image size depending on the component it is housed in
     * inside a [ImageAppCard] such as Image, Button, etc
     * If an object doesn't contain an [com.android.car.appcard.component.Image],
     * return `null`
     */
    fun getMaxImageSize(c: Class<*>): Size {
      return when (c) {
        ...
        else -> throw IllegalStateException("Maximum image size requested for unrecognized class")
      }
    }
  ...
  companion object {
    ...
    fun fromBundle(b: Bundle?): AppCardContext? {
      ...
      val fastRefreshPeriod = b.getInt(BUNDLE_KEY_FAST_REFRESH_PERIOD, refreshPeriodDefaultValue)
