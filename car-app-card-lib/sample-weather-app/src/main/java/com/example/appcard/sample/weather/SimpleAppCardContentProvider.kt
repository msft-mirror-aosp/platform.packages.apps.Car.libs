...
class SimpleAppCardContentProvider :
    ...
    override fun onCreate(): Boolean {
        ...
        context?.let {
            weatherAppCardProvider =
                WeatherAppCardProvider(it.applicationContext, WEATHER_ID, appCardUpdater)
        }

        Car.createCar(context, null, Car.CAR_WAIT_TIMEOUT_DO_NOT_WAIT) { car: Car?, ready: Boolean
            ->
            if (ready) {
    ...
    override val appCardIds: List<String> = listOf(WEATHER_ID).toMutableList()
    ...
    override fun onChangeEvent(newVehicleUnit: CarPropertyValue<*>?) {
        ...
        if (
            newVehicleUnit.value != VehicleUnit.FAHRENHEIT &&
