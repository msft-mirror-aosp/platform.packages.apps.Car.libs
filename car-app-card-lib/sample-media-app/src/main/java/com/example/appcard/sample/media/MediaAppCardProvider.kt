...
internal class MediaAppCardProvider(
    ...
    private val mediaSourceObserver = Observer<Any> { updateModel() }
    ...
    private fun getActiveAppCard(): ImageAppCard {
        ...
        val image =
            Image.newBuilder(IMAGE_ID)
                ...
                .setImageData(
                    drawableToBitmap(albumImageBitmap!!, maxArtSize!!.width, maxArtSize!!.height)
                )
                .build()
        ...
        val appImage =
            Image.newBuilder(HEADER_IMAGE_ID)
                ...
                .setImageData(
                    drawableToBitmap(
                        appIcon!!,
                        maxHeaderIconSize!!.width,
                        maxHeaderIconSize!!.height,
                    )
                )
                .build()
        ...
        return ImageAppCard.newBuilder(appCardId)
            .setImage(image)
            .setHeader(
                Header.newBuilder(HEADER_ID).setTitle(appName.toString()).setImage(appImage).build()
            )
    ...
    private fun getBlankAppCard(): ImageAppCard {
        val drawable =
            AppCompatResources.getDrawable(context, R.drawable.ic_zero_state)
                ?: return ImageAppCard.newBuilder(appCardId)
        ...
        val builder =
            ImageAppCard.newBuilder(appCardId)
                .setImage(image)
                .setPrimaryText(BLANK_PRIMARY_TEXT)
                .setSecondaryText(BLANK_SECONDARY_TEXT)

        getBlankAppCardHeader()?.let { builder.setHeader(it) }
    ...
    private fun getBlankAppCardHeader(): Header? {
        ...
        val headerSide =
            headerSize?.let { Math.min(it.width, it.height) } ?: DEFAULT_HEADER_IMAGE_SIDE

        val headerDrawable =
            AppCompatResources.getDrawable(context, R.drawable.ic_music) ?: return null
        ...
        return Header.newBuilder(HEADER_ID).setImage(headerImage).build()
    }
    ...
    private fun mediaSourceChanged(): Boolean {
        ...
        if (
            mediaSource != null &&
                (appName !== mediaSource.getDisplayName(context) || appIcon !== mediaSource.icon)
        ) {
            if (Log.isLoggable(TAG, Log.DEBUG)) {
    ...
    private fun metadataChanged(): Boolean {
        ...
        return metadata != null &&
            (songTitle !== metadata.title || artistName !== metadata.subtitle)
    }
